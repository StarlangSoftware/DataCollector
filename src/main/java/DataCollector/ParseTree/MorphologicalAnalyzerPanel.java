...
public class MorphologicalAnalyzerPanel extends LeafEditorPanel {
    ...
    public MorphologicalAnalyzerPanel(String path, String fileName, FsmMorphologicalAnalyzer fsm, boolean defaultFillEnabled) {
        ...
        tree.addTreeSelectionListener(e -> {
            ...
            if (node != null && node.getLevel() == fsmParses.length){
                ...
                MorphologicalAnalysisAction action = new MorphologicalAnalysisAction(((MorphologicalAnalyzerPanel) tree.getParent().getParent().getParent()), previousNode.getLayerInfo(), word1, word2);
                setAction(action);
                tree.setVisible(false);
            } else {
                if (node != null && node.getLevel() == 0){
                    LayerClearAction action = new LayerClearAction(((MorphologicalAnalyzerPanel) tree.getParent().getParent().getParent()), previousNode.getLayerInfo(), ViewLayerType.INFLECTIONAL_GROUP);
                    setAction(action);
                    tree.setVisible(false);
                }
            }
