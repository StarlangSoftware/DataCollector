...
import WordNet.Annotation.SynSetMergePanel;
...
/**
 * <p> The SynSet Merger is a tool enabling synonym literal matching in the target languages. This interface
 ...
 * process.</p>
 *
 * <p> The SynSet Merger is a practical option for matching intralingual synonym literals. However, transitivity
 ...
 * presents the editor Synset Splitter. Such overgrown synsets with weak or absent synonym relations between its
 * literals can be viewed and edited in the Synset Splitter by using split processes.</p>
 */
public class SynSetMergeFrame extends DataCollector{
     ...
     */
    public SynSetMergeFrame(final WordNet wordNet){
        this.setTitle("SynSet Merger");
        ...
        itemOpen.addActionListener(e -> {
            ...
            if (returnVal == JFileChooser.APPROVE_OPTION) {
                SynSetMergePanel literalMatcherPanel = new SynSetMergePanel(wordNet, fcinput.getSelectedFile().getParent() + "/" + fcinput.getSelectedFile().getName());
        ...
        itemSave.addActionListener(e -> {
            SynSetMergePanel current = (SynSetMergePanel) ((JScrollPane) projectPane.getSelectedComponent()).getViewport().getView();
            if (current != null){
                current.save();
            }
        });
