...
public class TurkishDependencyViewerPanel extends JPanel {
    ...
    public TurkishDependencyViewerPanel(String fileName){
    ...
    private void paintWord(Graphics g, TurkishDependencyTreeBankWord word, int index, int y){
        ...
        g.drawString(word.getName(), (int) ((index + 0.5) * TurkishDependencyViewerPanel.WORD_WIDTH - 0.5 * stringSize), y);
        if (word.getRelation() != null){
            stringSize = g.getFontMetrics().stringWidth(word.getRelation().toString());
            relationHeight = (word.getRelation().to() - index) * TurkishDependencyViewerPanel.RELATION_HEIGHT;
            pointStart = new Point2D.Double((index + .5) * TurkishDependencyViewerPanel.WORD_WIDTH, y - Math.signum(word.getRelation().to() - index) * TurkishDependencyViewerPanel.RELATION_HEIGHT / 2);
            pointCtrl1 = new Point2D.Double((index + .75) * TurkishDependencyViewerPanel.WORD_WIDTH, y - relationHeight);
            pointCtrl2 = new Point2D.Double((word.getRelation().to() + .25) * TurkishDependencyViewerPanel.WORD_WIDTH, y - relationHeight);
            pointEnd = new Point2D.Double((word.getRelation().to() + .5) * TurkishDependencyViewerPanel.WORD_WIDTH, y - Math.signum(word.getRelation().to() - index) * TurkishDependencyViewerPanel.RELATION_HEIGHT / 2);
    ...
    private void paintSentence(Graphics g){
        for (int i = 0; i < currentSentence.wordCount(); i++){
            ...
            paintWord(g, word, i, (currentSentence.maxDependencyLength() + 1) * TurkishDependencyViewerPanel.RELATION_HEIGHT);
        }
    }
